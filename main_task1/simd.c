...
TREE_BF* init_tree(size_t size, size_t threshold) {
    ...
    float* workspace = aligned_alloc(alignment, 21 * block_len*sizeof(float));
    if (!workspace) {
        free(node);
        return NULL;
    }
...
for (int i = 0; i < size; i++) {
    for (int j = 0; j < size; j++) {
        ...
        for (; k + 7 < size; k += 8) {
            __m256 a = _mm256_loadu_ps(&A[i * total_size + k]);
            __m256 b = _mm256_loadu_ps(&F[j * total_size + k]);
            __m256 mul = _mm256_mul_ps(a, b);
            sum256 = _mm256_add_ps(sum256, mul);
        }
...
int main(int argc, char** argv){
    ...
    float* A = aligned_alloc(alignment, len*sizeof(float));
    float* B = aligned_alloc(alignment, len*sizeof(float));
    float* F = aligned_alloc(alignment, len*sizeof(float));
    float* D = aligned_alloc(alignment, len*sizeof(float));
    ...
    fprintf(file_LOG, "SIMD+PARALLEL+STRAS+TRANS:%d %.9lf ", size, time_dif(ts[0], ts[1]));
